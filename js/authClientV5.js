...
export async function getAuthClient(){
  ...
  if(!clientPromise){
    const timeout=Math.max(8000,Math.min(45000,Number(config.security?.requestTimeoutMs||20000)));
    clientPromise=import('https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm').then(({createClient})=>createClient(
      source.url,
      source.publishableKey||source.anonKey,
      {
        auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true,flowType:'pkce'},
        global:{fetch:timeoutFetch(timeout),headers:{'X-Client-Info':'linuxaid-web'}}
      }
    ));
  }
...
export async function signInGitHub(){const c=await getAuthClient();const {data,error}=await c.auth.signInWithOAuth({provider:'github',options:{redirectTo:site('dashboard.html'),scopes:'read:user user:email'}});if(error)throw error;return data}
